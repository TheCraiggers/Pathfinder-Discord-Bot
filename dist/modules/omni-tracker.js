...
const bundle_js_1 = require("rpg-dice-roller/lib/umd/bundle.js");
const helpMessages_1 = require("./helpMessages");
...
class Character {
    ...
    resolveReference(stuff, roller, depth) {
        ...
        return { result: mathjs_1.compile(stuff).evaluate(), humanReadable: humanReadable };
    }
    /**
        ...
        for (let property of properties) {
            ...
            if (property.groups.propertyValue) {
                ...
                if (property.groups.propertyValue.startsWith("=")) {
                    ...
                    let roller = new bundle_js_1.DiceRoller();
...
class OmniTracker {
    ...
    static handleCommonErrors(message, error) {
        if (error instanceof OmniError) {
            ...
            console.error(error);
            message
                .reply("Sorry, an unknown error occurred. Please check your command syntax.")
                .catch(err => {
                console.error(err);
            });
        }
    ...
    sortCharsByInit() {
        ...
        return Object.keys(foo).sort(function (a, b) {
            ...
            if (foo[a].properties["initiative"].currentValue ==
                ...
                return (foo[b].properties["initiative"].currentValue -
    ...
    increaseTime(stringDuration, message) {
        ...
        const oldTime = new moment_1.default(this.time).utc();
        ...
        if (parsed.groups.duration) {
            var duration = moment_1.duration(parseInt(parsed.groups.durationValue), parsed.groups.durationUnits);
            this.time.add(duration);
        }
        else if (parsed.groups.specific) {
            var duration = moment_1.duration(parsed.groups.specific);
            this.time.add(duration);
        }
            ...
            switch (parsed.groups.abstract.toUpperCase()) {
                ...
                case "MORNING":
                    ...
                    var duration = moment_1.duration(this.time.diff(oldTime));
                    break;
                default:
                    message
                        .reply(`Sorry, I don't know how to set time to ${parsed.groups.abstract} yet.`)
                        .catch(console.error);
                    break;
            }
        }
...
function handleCommand(message) {
    ...
    switch (keyword) {
        case "omni help":
            message.author
                .send(helpMessages_1.default.general)
                .then(function () {
                message.author.send(helpMessages_1.default.example);
            })
                .then(function () {
                message.author.send(helpMessages_1.default.gm);
            })
                .catch(console.error);
...
function handleTrackerCommands(command, message) {
    switch (command.groups.verb) {
        ...
        default:
            message
                .reply(`Sorry, I don't know how to ${command.groups.verb} a ${command.groups.noun} yet.`)
                .catch(console.error);
    }
}
            ...
            gmOnlyCommand(message)
                ...
                if (command.groups.target == "%enemies") {
                    ...
                    if (character) {
                        ...
                        message
                            .reply(`Couldn't find a character with the name ${command.groups.target}. Please check your spelling!`)
                            .catch(error => {
                            console.error(error);
                        });
                        return;
            ...
            OmniTracker.getBotDataMessages(message).then(data => {
                ...
                try {
                    ...
                    for (property of propertyNamesToDelete) {
                        ...
                        message
                            .reply(`Removing property ${property.groups.propertyName} from character ${characterName}`)
                            .catch(err => {
                            console.error(err);
                        });
                    }
        ...
        case "roll":
            OmniTracker.getTracker(message)
                ...
                if (stuffToRoll.groups.destinationStat) {
                    ...
                    message
                        .reply(`\`\`\`${stuffToRoll.groups.sourceStat} for ${character.name} is ${output.result};${stuffToRoll.groups.rollComment}\n${output.humanReadable}\`\`\``)
                        .catch(console.error);
                }
            })
                .catch(error => {
    ...
    if (!command) {
        ...
        OmniTracker.getBotDataMessages(message)
            ...
            if (command.groups.destinationStat) {
                ...
                message
                    .reply(`\`\`\`${command.groups.destinationStat} for ${character.name} has been set to ${output.result};${command.groups.rollComment}\n${output.humanReadable}\`\`\``)
                    .catch(console.error);
            }
            else {
                message
                    .reply(`\`\`\`${command.groups.sourceStat} for ${character.name} is ${output.result};${command.groups.rollComment}\n${output.humanReadable}\`\`\``)
                    .catch(console.error);
            }
            tracker.saveBotData();
            tracker.updateTrackers();
        ...
        if (tracker.combatCurrentInit === undefined) {
            ...
            for (let i = 0; i < sortedCharacterNames.length; i++) {
                if (sortedCharacterNames[i] == tracker.combatCurrentInit) {
                    if (i + 1 >= sortedCharacterNames.length ||
                        tracker.characters[sortedCharacterNames[i + 1]].properties["initiative"] == undefined) {
