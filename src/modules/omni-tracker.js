...
import { DiceRoller } from "rpg-dice-roller/lib/umd/bundle.js";
...
export default class OmniPlugin {
  constructor(client) {
...
class OmniError extends Error {
  constructor(message) {
    if (!message) {
      message =
        "An unknown error occurred. Check your command syntax. If you're sure it's right, contact the bot administrator.";
    }
    super(message);
...
class PropertyNotFoundError extends OmniError {
  constructor(propertyName, characterName) {
    super(
      `The property "${propertyName}" was not found on character ${characterName}.`
    );
    this.name = this.constructor.name;
  }
}
class EffectNotFoundError extends OmniError {
  constructor(effectName, characterName) {
    super(
      `The effect "${effectName}" was not found on character ${characterName}.`
    );
    this.name = this.constructor.name;
  }
}
class Property {
  ...
  static newPropertyFromBotData(botData) {
    if (botData.maxValue) {
      return new PropertyRange(
        botData.propertyName,
        botData.currentValue,
        botData.maxValue,
        botData.isAboveFold,
        botData.character,
        botData.dataMessage
  ...
  delete() {
    this.dataMessage.delete().catch(error => {
      console.error(error);
    });
  }
...
class Effect {
  ...
  constructor({
    ...
    affectedCharacterName = undefined,
...
class Character {
  ...
  resolveReference(stuff, roller, depth) {
    ...
    for (const propertyToEval of propertiesToEval) {
      ...
      let resolved = this.resolveReference(
      ...
      humanReadable = humanReadable.replace(
        propertyToEval[0],
        resolved.humanReadable
      ); //I don't use parens here because it's less readable
      stuff = stuff.replace(propertyToEval[0], "(" + resolved.result + ")");
    }

    return { result: compile(stuff).evaluate(), humanReadable: humanReadable };
  }
  ...
  addProperty(propertiesString, message) {
    ...
    for (let property of properties) {
      property.groups.propertyName = Property.translateAliasedPropertyNames(
        property.groups.propertyName
      );
      let important = false;
      if (property.groups.important) important = true;
      if (property.groups.propertyValue) {
        //Is it a PropertyRange or just a Property?
        if (property.groups.propertyValue.startsWith("=")) {
          property.groups.propertyValue = property.groups.propertyValue.replace(
            "=",
            ""
          ); //Dynamic stats are set as is- no resolving.
        } else {
          let roller = new DiceRoller();
          property.groups.propertyValue = this.resolveReference(
            property.groups.propertyValue,
            roller
          ).result;
        ...
        promises.push(
          this.setPropertyRange(
            property.groups.propertyName,
            property.groups.propertyMinValue,
            property.groups.propertyMaxValue,
            important
          )
        );
      }
      if (message) {
        if (property.groups.propertyName.toLowerCase() == "hp") {
          ...
          reply += `Property ${
            this.properties[property.groups.propertyName.toLowerCase()]
          }\n`;
        }
      }
    }
    Promise.all(promises).then(results => {
      ...
      if (reply) {
        message
          .reply(`The following properties were set:\n${reply}`)
          .then(msg => {
            msg.delete(20000);
          })
          .catch(error => {
  ...
  dealDamage(amountOfDamage) {
    ...
    if (temphp) {
      ...
      if (temphp.currentValue <= 0) {
        ...
        return this.setHealth(this.HP.currentHP * 1 + temphp.currentValue)
          .then(() => {
  ...
  setHealth(currentHP, maxHP) {
    if (maxHP !== undefined) this.HP.maxHP = maxHP;
  ...
  setProperty(propertyName, value, isAboveFold) {
    ...
    switch (
      propertyName.toLowerCase() //Some props are so important they exist on the char object. Deal with those.
    ) {
      case "hp":
      ...
      default:
        ...
        if (property) {
          ...
          return this.dataMessage.channel
            .send(JSON.stringify(property))
            .then(msg => {
  ...
  showCharacterSynopsis(channel) {
    ...
    for (var propertyName of Object.keys(this.properties)) {
      ...
      if (property.isAboveFold) output += ` ${property.toString()}`;
    }
  ...
  addEffect(effectName, durationString) {
    ...
    let durationInSeconds = Effect.translateDurationStringToSeconds(
      durationString
    );
    ...
    if (existingEffect) {
      ...
      let newEffect = (this.effects[effectName.toLowerCase()] = new Effect({
        effectName: effectName,
        durationInSeconds: durationInSeconds,
        affectedCharacterName: this.name,
        endOfTurnEffect: endOfTurnEffect
      }));
...
class OmniTracker {
  ...
  static getBotDataMessages(message) {
    return new Promise(function(resolve, reject) {
      ...
      let botDatum = [];
      var botDataChannel = message.guild.channels.find(
        msg => msg.name == "omni-data"
      );
      if (!botDataChannel) {
        ...
        (async function getAllMessages() {
          ...
          while (lastCollection.size > 0) {
            for (const msg of lastCollection) {
              ...
              botDatum.push(data);
              if (data.type == "OmniTracker") var omniData = true;
            }
  ...
  static handleCommonErrors(message, error) {
    if (error instanceof OmniError) {
      message.reply(error.message).catch(err => {
        console.error(err);
      });
    } else if (error.constructor.name == "peg$SyntaxError") {
      message
        .reply("Invalid dice notation. Please check your syntax.")
        .catch(err => {
          console.error(err);
        });
    } else {
      ...
      message
        .reply(
          "Sorry, an unknown error occurred. Please check your command syntax."
        )
        .catch(err => {
          console.error(err);
        });
  ...
  constructor(botData) {
    ...
    for (const characterToImport of charactersToImport) {
      this.characters[
        characterToImport.name.toLowerCase()
      ] = Character.importFromBotData(characterToImport);
    }
      ...
      switch (data.type) {
        case "Property":
          if (this.characters[data.character.toLowerCase()]) {
            this.characters[data.character.toLowerCase()].properties[
              data.propertyName.toLowerCase()
            ] = Property.newPropertyFromBotData(data);
        ...
        case "Effect":
          if (this.characters[data.affectedCharacterName.toLowerCase()]) {
            this.characters[data.affectedCharacterName.toLowerCase()].effects[
              data.effectName.toLowerCase()
            ] = Effect.importFromBotData(data);
          }
          break;
  ...
  toJSON() {
    return {
      type: "OmniTracker",
      date: this.time,
      combatCurrentInit: this.combatCurrentInit
    };
  }

  save() {
    this.omniDataMessage.edit(JSON.stringify(this)).catch(error => {
      console.error(error);
    });
  }
  ...
  sortCharsByInit() {
    ...
    return Object.keys(foo).sort(function(a, b) {
      ...
      if (
        foo[a].properties["initiative"].currentValue ==
        foo[b].properties["initiative"].currentValue
      ) {
        if (foo[a].enemy) {
  ...
  addNewCharacter(name, owner, characterClass) {
    ...
    return new Promise(function(resolve, reject) {
      if (characterClass.prototype instanceof Character) {
        let newCharacter = new characterClass({
          name: name,
          owner: owner,
          HP: { currentHP: 0, maxHP: 0 }
        });
        tracker.omniDataMessage.channel
          .send(JSON.stringify(newCharacter))
  ...
  increaseTimeForCharacter(increaseInSeconds, endOfTurn, character, message) {
    ...
    if (expiredEffectsMessage) {
      message.channel.send(expiredEffectsMessage).catch(console.error);
    }
  ...
  increaseTime(stringDuration, message) {
    ...
    if (parsed.groups.duration) {
      var duration = _duration(
        parseInt(parsed.groups.durationValue),
        parsed.groups.durationUnits
      );
      this.time.add(duration);
    } else if (parsed.groups.specific) {
      var duration = _duration(parsed.groups.specific);
      this.time.add(duration);
      ...
      switch (parsed.groups.abstract.toUpperCase()) {
        ...
        case "MORNING":
          ...
          var duration = _duration(this.time.diff(oldTime));
          break;
        default:
          message
            .reply(
              `Sorry, I don't know how to set time to ${parsed.groups.abstract} yet.`
            )
            .catch(console.error);
          break;
      }
    ...
    message
      .reply(`Forwarding time ${oldTime.from(this.time, true)}`)
      .catch(console.error);

    for (let characterName in this.characters) {
      const character = this.characters[characterName];
      this.increaseTimeForCharacter(
        increaseInSeconds,
        undefined,
        character,
        message
  ...
  saveBotData() {
    ...
    return;
    this.omniDataMessage.edit(JSON.stringify(this)).catch(console.error);
    ...
    for (var characterName in this.characters) {
      ...
      if (!character.dataMessage) {
        ...
        character.dataMessage.edit(json).catch(console.error);
      }
    }
  }
          ...
          .then(messages => {
            for (var msg of messages) {
              if (msg[1].content.startsWith("```CSS\n[Omni Tracker]")) {
                msg[1].edit(updatedTracker).catch(console.error);
              } else if (
                msg[1].content.startsWith("```CSS\n[Omni GM Tracker]")
              ) {
                msg[1].edit(updatedGMTracker).catch(console.error);
              }
            }
          })
          .catch(console.error);
  ...
  generateOmniTrackerMessageText(isGMTracker) {
    ...
    for (var characterName in characters) {
      ...
      if (this.combat) {
        ...
        if (
          this.combatCurrentInit &&
          this.combatCurrentInit.toLowerCase() == character.name.toLowerCase()
        ) {
          output += `> ${init} | `;
        } else {
          output += `  ${init} | `;
      ...
      for (var effectName of Object.keys(character.effects)) {
        ...
        output += `${combatIndent}${character.indent} ${
          effect.effectName
        } ${effect.getDurationText()}\n`;
      }
    }
  ...
  getCharacterFromAuthorID(ID) {
    if (
      this.combat &&
      this.characters[this.combatCurrentInit] &&
      this.characters[this.combatCurrentInit].owner == ID
    ) {
...
function handleCommand(message) {
  ...
  const keyword = message.content
    .match(botCommandRegex)
    .groups.keyword.toLowerCase();
  switch (keyword) {
    case "omni help":
      message.author
        .send(HelpMessages.general)
        .then(function() {
          message.author.send(HelpMessages.example);
        })
        .then(function() {
          message.author.send(HelpMessages.gm);
        })
        .catch(console.error);
    ...
    case "omni":
      ...
      if (!command) {
        message
          .reply("Invalid !omni command. Use !omni help if needed.")
          .catch(console.error);
        return;
      }
...
function handleTrackerCommands(command, message) {
  switch (command.groups.verb) {
      ...
      OmniTracker.getBotDataMessages(message)
        .then(function(data) {
          ...
          return message.channel.send(
            omniTracker.generateOmniTrackerMessageText(isGMTracker)
          );
        })
    ...
    case "show":
      OmniTracker.getBotDataMessages(message)
        .then(function(data) {
          ...
          omniTracker = new OmniTracker(data);
          return message.channel.send(
            omniTracker.generateOmniTrackerMessageText()
          );
        })
        .catch(console.error);
      break;
    default:
      message
        .reply(
          `Sorry, I don't know how to ${command.groups.verb} a ${command.groups.noun} yet.`
        )
        .catch(console.error);
  }
}
        ...
        .then(data => {
          ...
          if (command.groups.target == "%enemies") {
            ...
            if (character) {
              ...
              message
                .reply(
                  `Couldn't find a character with the name ${command.groups.target}. Please check your spelling!`
                )
                .catch(error => {
                  console.error(error);
                });
      ...
      OmniTracker.getBotDataMessages(message)
        .then(data => {
          ...
          if (command.groups.noun == "player") {
            ...
            throw new OmniError(
              "Invalid command. Need to specify if you're adding a Player or an Enemy!"
            );
          }
          ...
          tracker
            .addNewCharacter(characterName, message.author.id, characterType)
            .then(newCharacter => {
              ...
              ].showCharacterSynopsis(message.channel);
            });
        })
        .catch(error => {
    ...
    case "show":
      OmniTracker.getBotDataMessages(message)
        .then(data => {
          var tracker = new OmniTracker(data);
          let character =
            tracker.characters[command.groups.target.toLowerCase()];
          if (!character) {
            ...
            output += "```";
            message.reply(output).catch(error => {
              console.error(error);
            });
          }
        })
  ...
  switch (command.groups.verb) {
    case "add":
      OmniTracker.getBotDataMessages(message)
        .then(data => {
          var tracker = new OmniTracker(data);
          var characterName = command.groups.target
            .toLowerCase()
            .replace(/'/g, "");
          ...
          if (effect) {
            effect.groups.effectName = effect.groups.effectName.replace(
              /'/g,
              ""
            );
            switch (command.groups.target.toLowerCase()) {
              case "%players":
                for (characterName in tracker.characters) {
                  ...
                  if (!character.enemy) {
                    character.addEffect(
                      effect.groups.effectName,
                      effect.groups.durationInfo
                    );
              ...
              case "%all":
                ...
                break;
              default:
                tracker.characters[characterName].addEffect(
                  effect.groups.effectName,
                  effect.groups.durationInfo
                );
                tracker.characters[characterName].showCharacterSynopsis(
                  message.channel
                );
                break;
            }
            ...
            message.reply("Invalid effect command.").catch(console.error);
          }
        })
        .catch(console.error);
      break;
          ...
          if (effect) {
            ...
            switch (command.groups.target.toLowerCase()) {
              ...
              case "%all":
                ...
                break;
              default:
                tracker.characters[characterName.toLowerCase()].removeEffect(
                  effect.groups.effectName.toLowerCase()
                );
                tracker.characters[
                  characterName.toLowerCase()
                ].showCharacterSynopsis(message.channel);
...
function handleTimeCommands(command, message) {
  gmOnlyCommand(message).then(function() {
    switch (command.groups.verb) {
      case "add":
        OmniTracker.getBotDataMessages(message)
          .then(data => {
  ...
  switch (command.groups.verb) {
    ...
    case "set":
      OmniTracker.getBotDataMessages(message)
        .then(data => {
          ...
          if (tracker.characters[characterName]) {
            tracker.characters[characterName].addProperty(
              command.groups.properties,
              message
            );
            tracker.saveBotData();
            tracker.updateTrackers();
            ...
            message
              .reply(`Player ${command.groups.target} could not be found.`)
              .catch(console.error);
          }
        })
        .catch(error => {
          console.error(error);
          message.reply(
            "Sorry, an error was encountered. Please check your command!"
          );
        });
      break;
    ...
    case "remove":
      OmniTracker.getBotDataMessages(message).then(data => {
        ...
        try {
          let characterName = command.groups.target
            .toLowerCase()
            .replace(/'/g, "");
          let character = tracker.characters[characterName];
          if (!character) {
          ...
          let propertyNamesToDelete = command.groups.properties
            .toLowerCase()
            .matchAll(/(?<propertyName>\w+)/g);
          for (property of propertyNamesToDelete) {
            ...
            message
              .reply(
                `Removing property ${property.groups.propertyName} from character ${characterName}`
              )
              .catch(err => {
                console.error(err);
              });
      ...
      break;
    case "roll":
      OmniTracker.getTracker(message)
        .then(tracker => {
          let roller = new DiceRoller();
          let character =
            tracker.characters[command.groups.target.toLowerCase()];
          ...
          const stuffToRoll = command.groups.properties.match(
            rollPropertyRegex
          );
          const output = character.resolveReference(
            stuffToRoll.groups.sourceStat,
            roller
          );
          ...
          if (stuffToRoll.groups.destinationStat) {
            character
              .setProperty(stuffToRoll.groups.destinationStat, output.result)
              .then(function() {
                message.reply(
                  `\`\`\`${stuffToRoll.groups.destinationStat} for ${character.name} has been set to ${output.result};${stuffToRoll.groups.rollComment}\n${output.humanReadable}\`\`\``
                );
              })
              .catch(console.error);
          } else {
            message
              .reply(
                `\`\`\`${stuffToRoll.groups.sourceStat} for ${character.name} is ${output.result};${stuffToRoll.groups.rollComment}\n${output.humanReadable}\`\`\``
              )
              .catch(console.error);
          }
        })
...
function handleRollAliasCommands(message) {
  ...
  if (!command) {
    message.reply("Invalid command!").catch(console.error);
  } else {
    OmniTracker.getBotDataMessages(message)
      .then(data => {
        var tracker = new OmniTracker(data);
        ...
        if (command.groups.destinationStat) {
          ...
          message
            .reply(
              `\`\`\`${command.groups.destinationStat} for ${character.name} has been set to ${output.result};${command.groups.rollComment}\n${output.humanReadable}\`\`\``
            )
            .catch(console.error);
        } else {
          message
            .reply(
              `\`\`\`${command.groups.sourceStat} for ${character.name} is ${output.result};${command.groups.rollComment}\n${output.humanReadable}\`\`\``
            )
            .catch(console.error);
        }
        tracker.saveBotData();
      ...
      .catch(error => {
        ...
        if (!notation) {
          ...
          try {
            ...
            console.error(error);
            message.reply("Invalid roll command!").catch(console.error);
          }
        }
      });
  }
  ...
  OmniTracker.getBotDataMessages(message)
    .then(data => {
      ...
      if (tracker.combatCurrentInit === undefined) {
        ...
        tracker.increaseTimeForCharacter(
          6,
          true,
          tracker.characters[tracker.combatCurrentInit],
          message
        );
        for (let i = 0; i < sortedCharacterNames.length; i++) {
          if (sortedCharacterNames[i] == tracker.combatCurrentInit) {
            if (
              i + 1 >= sortedCharacterNames.length ||
              tracker.characters[sortedCharacterNames[i + 1]].properties[
                "initiative"
              ] == undefined
            ) {
      ...
      message.channel.send(
        `Hey <@${character.owner}> it's ${character.name}${suffix} turn!`
      );
    ...
    .catch(error => {
      console.error(error);
      message
        .reply("Something went wrong when changing init. What did you do?!")
        .catch(console.error);
    });
}
            ...
            character
              ...
              .then(() => {
                ...
                return message.reply(
                  `Enemy ${character.name} removed from combat.`
                );
              })
              .then(() => {
                ...
                if (!enemiesAlive) {
                  ...
                  for (characterName in tracker.characters) {
                    if (
                      tracker.characters[characterName].properties["initiative"]
                    ) {
                      tracker.characters[characterName].properties[
                        "initiative"
                      ].delete();
                      delete tracker.characters[characterName].properties[
                        "initiative"
                      ];
                    }
                  }
...
function handleOmniSetup(message) {
  ...
  let botChannelCategory = message.guild.channels.find(
    category => category.name == channelCategoryName
  );
  let botDataChannel = message.guild.channels.find(
    msg => msg.name == dataChannelName
  );

  if (!gmRole) {
    message.guild.createRole({ name: "GM" }).catch(error => {
      console.error(error);
    });
  }
  ...
  new Promise(function(resolve, reject) {
    if (botChannelCategory) {
      ...
      resolve(
        message.guild.createChannel(channelCategoryName, {
          type: "category",
